...
public final class IronSessionAPIClient: @unchecked Sendable {
    ...
    ///   - files: File attachments with field name, filename, data, and content type
    /// - Returns: Response data
    /// - Throws: API errors if request fails
    public func authenticatedMultipartRequest(
        path: String,
        fields: [String: String] = [:],
        files: [(fieldName: String, filename: String, data: Data, contentType: String)] = []
    ) async throws -> Data {
        ...
        logger.log("🌐 Making authenticated multipart request to \(path), body size: \(body.count) bytes", level: .debug, category: .network)
        ...
        if httpResponse.statusCode == 401 {
            logger.log("🔐 Received 401 on multipart request, attempting refresh and retry", level: .debug, category: .network)
            ...
            guard 200...299 ~= retryHTTPResponse.statusCode else {
                throw AuthenticationError.apiError(retryHTTPResponse.statusCode, "HTTP \(retryHTTPResponse.statusCode)")
            }
        ...
        guard 200...299 ~= httpResponse.statusCode else {
            throw AuthenticationError.apiError(httpResponse.statusCode, "HTTP \(httpResponse.statusCode)")
        }
